...
//System.out.printf("B=%s,c=%c i=%d, total=%d\n",binary, binary.charAt(i), i, decimal);
	...
	static void DtoH() {}
	static void HtoD() {
		String hex = JOptionPane.showInputDialog(null, "Please enter a hexadecimal number","Hexadecimal to Decimal",JOptionPane.INFORMATION_MESSAGE);
		int decimal = 0;
		int len = hex.length();
		//handle CANCEL option
		if(hex == "") return;
		...
		for (int i = len-1; i >= 0; i--) {
			if(hex.charAt(i)>='0' && hex.charAt(i)<='9') {
				decimal += ((hex.charAt(i)-'0')*Math.pow(16,len-1-i));
		   }else if(hex.charAt(i)>='A' && hex.charAt(i)<='F') {
				decimal += ((hex.charAt(i)-'A'+10)*Math.pow(16,len-1-i));
		   }else{
			   JOptionPane.showMessageDialog(null,"Error. You can only enter hexadecimal numbers" ,"Hexadecimal conversion error", JOptionPane.ERROR_MESSAGE);
				return;
		   }
//System.out.printf("B=%s,c=%c i=%d, total=%d\n",binary, binary.charAt(i), i, decimal);
		}
		JOptionPane.showMessageDialog(null,hex + " converted to decimal is " + decimal ,"Hexadecimal conversion complete", JOptionPane.INFORMATION_MESSAGE);
	}
} // end of class
